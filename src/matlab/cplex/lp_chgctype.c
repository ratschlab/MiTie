...
)
{
  ...
  if (nlhs !=0 || nrhs != 4) {
    mexErrMsgTxt("Usage: lp_chgctype(lpenv, p_lp, idx, type)");
    return;
  }
  ...
  if ( lp != NULL ) {
    status = CPXchgctype(env, lp, 1, &idx, &type);
    if ( status ) {
      fprintf (stderr, "CPXchgctype failed, error code %d.\n", status);
    }
  }
