...
)
{
  ...
  if (nrhs > 5 || nrhs < 1) {
    mexErrMsgTxt("Usage: [how] "
		 "= lp_delcols(lpenv,p_lp,from,to,disp)");
    return;
  }
  switch (nrhs) {
  case 5:
  ...
  case 4:
	...
	mexErrMsgTxt("4th argument (to) must be "
		     "an integer scalar.");
	return;
      }
      to = (int)*mxGetPr(prhs[3]);
    }
  case 3:
	...
	mexErrMsgTxt("3rd argument (from) must be "
		     "an integer scalar.");
	return;
      }
      from = (int)*mxGetPr(prhs[2]);
    }
  case 2:
  ...
  if (display>3) fprintf(STD_OUT, "(display=%i, from=%i, to=%i) \n", display, from, to) ;
  ...
  if (display>2) 
    fprintf(STD_OUT, "calling CPXdelcols\n") ;
  status = CPXdelcols(env, lp, (int)from, (int)to);
  if ( status ) {
    fprintf (STD_OUT, "CPXdelcols failed.\n");
    goto TERMINATE;
  }
