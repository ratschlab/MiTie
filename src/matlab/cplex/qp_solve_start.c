	...
	)
{
	...
	if (nrhs > 11 || nrhs < 1) {
		mexErrMsgTxt("Usage: [x,lambda,how,p_qp] "
					 "= qp_solve(cpenv,Q,c,A,b,l,u,neq,disp,method,x0)");
		return;
	}
	switch (nrhs) {
	case 11:
		if (mxGetM(prhs[10]) != 0 || mxGetN(prhs[10]) != 0) {
			if (!mxIsNumeric(prhs[10]) || mxIsComplex(prhs[10]) 
				||  mxIsSparse(prhs[10])
				|| !mxIsDouble(prhs[10]) 
				||  mxGetN(prhs[10])!=1 ) {
				mexErrMsgTxt("11th argument (x0) must be a column vector.");
				return;
			}
			x0 = mxGetPr(prhs[10]);
			n = mxGetM(prhs[10]);
	...
	/* Set start value */
	if( x0 != NULL ) {
		int err = CPXsetintparam( env, CPX_PARAM_ADVIND, CPX_ON );
		if ( err ) {
			fprintf( STD_OUT, "CPXsetintparam failed.\n" );
			goto TERMINATE;
		}
		if( display > 2 ) {
			fprintf( STD_OUT, "calling CPXcopystart \n" );
		}
		s0 = myMalloc( m * sizeof(double) ) ;
		for( i = 0; i < m; ++i ) {
			s0[i] = 0.0;
		}
		/* status = CPXcopystart( env, lp, cstat, rstat, cprim, rprim, cdual, rdual ); */
		status = CPXcopystart( env, qp, NULL, NULL, x0, s0, NULL, NULL );
		myFree( s0 );
		if ( status ) {
			fprintf( STD_OUT, "CPXcopystart failed.\n" );
			goto TERMINATE;
		}
	}
